...
def assert_all_are_close(iterable_a, iterable_b):
    ...
    for a, b in zip_longest(iterable_a, iterable_b):
        if a is None or b is None:
            assert a == b, f"Iterables have different lengths: {list(iterable_a)} vs. {list(iterable_b)}"
        try:
            iter(a)
            iter(b)
...
@pytest.mark.parametrize(
    "channel_pitch_x", [1.0, 2.0, 3.0]
)  # Add your desired values here
@pytest.mark.parametrize(
    "channel_pitch_y", [1.0, 2.0, 3.0]
)  # Add your desired values here
@pytest.mark.parametrize(
...
def test_get_detector_numbers_simple_2x2_example(first_pixel_id, number_of_pixels_x, number_of_pixels_y):
    detector = BoxNXDetector(
        name="Detector",
        number_of_pixels_x=number_of_pixels_x,
        number_of_pixels_y=number_of_pixels_y,
    ...
    pixel_grid = [[3,4], [5,6]]
    detector_numbers = detector.get_detector_numbers()
...
def test_get_detector_numbers(first_pixel_id, number_of_pixels_x, number_of_pixels_y):
    ...
    pixels = [p for p in range(first_pixel_id, first_pixel_id + (number_of_pixels_x * number_of_pixels_y))]
    pixel_grid = [pixels[i:i+number_of_pixels_x] for i in range(0, len(pixels), number_of_pixels_x)]
    assert_all_are_close(pixel_grid, detector.get_detector_numbers())
...
def test_pixel_offsets_simple_4x2_example_with_gaps(first_pixel_id, number_of_pixels_x, number_of_pixels_y):
    detector = BoxNXDetector(
        name="Detector",
        number_of_pixels_x=number_of_pixels_x,
        number_of_pixels_y=number_of_pixels_y,
    ...
    expected_x_offsets = [ [-64.35, 64.35] for _ in range(4)]
    expected_y_offsets = [
        [192.25, 192.25],
        [64.25, 64.25],
        [-64.25, -64.25],
    ...
    expected_z_offsets = [ [0, 0] for _ in range(4)]

    for idx, offsets in enumerate(detector.get_x_pixel_offsets()):
        assert_all_are_close(expected_x_offsets[idx], offsets)
    for idx, offsets in enumerate(detector.get_y_pixel_offsets()):
        assert_all_are_close(expected_y_offsets[idx], offsets)
    for idx, offsets in enumerate(detector.get_z_pixel_offsets()):
        assert_all_are_close(expected_z_offsets[idx], offsets)
...
def test_render_3_detectors():
    def load_json(file_path):
        with open(file_path, 'r') as f:
            return json.load(f)
    json1 = load_json("examples/tests/json/nmx_render_3_detectors.json")
    json2 = json.loads(render())
    # assert_dicts_equal(json1, json2)
    assert json1 == json2
...
#     for k in d1.keys():
#         if k not in d2:
#             assert False, f"Key {k} not found in second json at {path}"
#         else:
#             if isinstance(d1[k], dict):
#                 path += f"{k}/"
#                 assert_dicts_equal(d1[k], d2[k], path)
#             elif d1[k] != d2[k]:
#                 path += f"{k}/"
#                 assert False, f"Different value found for key={k} at path={path}:\n\n\n{d1[k]} (first json)\n\n vs \n\n{d2[k]} (second json)"
