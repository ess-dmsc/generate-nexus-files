...
"""
Generates example file with geometry for TBL instrument with multiblade detector
...
SAMPLE_TO_CLOSEST_SUBSTRATE_EDGE_m = 0.0  # R on diagrams
WIRES_PER_BLADE = 32
STRIPS_PER_BLADE = 64
ANGLE_BETWEEN_SUBSTRATE_AND_NEUTRON_deg = 5.0  # theta on diagrams
NUMBER_OF_BLADES = 14  # Maybe only 6 currently with digitisers?
DISTANCE_BETWEEN_BLADES = 0.0105 # 10.5mm

INSTRUMENT_NAME = "TBL"
EVENT_TOPIC = "tbl_detector_mb"
EVENT_SOURCE_NAME = "multiblade"
...
def _position_blade(blade_number, vertices):
    ...
    for index, vertex in enumerate(vertices):
        ...
        # shift vertex in height by blade_number * 10.5mm

        vertex[1] += blade_number * DISTANCE_BETWEEN_BLADES
        transformed_vertices[index, :] = vertex
...
def add_shape_to_detector(
    ...
    shape_group = builder.add_nx_group(
        detector_group, "pixel_shape", "NXoff_geometry"
    )
    builder.add_dataset(shape_group, "vertices", vertices.astype(np.float64), {"units": "m"})
...
def create_detector_shape_info():
    ...
    total_vertices[:, 0] -= np.mean(total_vertices[:, 0])
    total_vertices[:, 1] -= np.mean(total_vertices[:, 1])
    total_vertices[:, 2] -= np.mean(total_vertices[:, 2])
...
def create_pixel_offsets():
    ...
    total_x_offsets -= np.mean(total_x_offsets)
    total_y_offsets -= np.mean(total_y_offsets)
    total_z_offsets -= np.mean(total_z_offsets)

    return total_x_offsets, total_y_offsets, total_z_offsets


def run_create_geometry(json_file_path="TBL_nexus_structure.json"):
